...
async def bp_get_suppliers(request):
    ...
    Role GUEST/USER: chỉ trả về thông tin cơ bản (name, phone, email, address, status).
    Role ADMIN: trả về toàn bộ thông tin kèm _id, code, created_at, updated_at.
    """
...
async def bp_update_supplier(request, supplier_id):
    ...
    try:
        ...
        # Get supplier by code to update
...
async def bp_delete_supplier(request, supplier_id):
    ...
    Ràng buộc:
    - Nhà cung cấp phải ở trạng thái inactive.
    - Không còn sản phẩm nào thuộc nhà cung cấp này.
    """
    try:
        supplier = supplier_repo.get_supplier_by_object_id(supplier_id)
        if not supplier:
